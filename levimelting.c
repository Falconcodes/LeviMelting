...
void main(void){
  ...
  //таймер, управляющий ШИМ для водяного насоса и для подстройки частоты
  TCCR2A=(1<<COM2A1) | (0<<COM2A0) | (1<<COM2B1) | (0<<COM2B0) | (1<<WGM21) | (1<<WGM20);
  TCCR2B=(0<<WGM22) | (1<<CS22) | (1<<CS21) | (0<<CS20);
  TCNT2=0;
  OCR2B=0;    //период для частоты (больше = частота ниже)
  OCR2A=40;   //период для насоса (больше = больше напор и громче работает)
...
//      }; 
 ...
 //===ЭТАП ПЕРВЫЙ. Прогрев пустого контура большим током      
  #asm("sei")
  while (i < (WARM_TIME*3)) { //ждем WARM_TIME секунд, пока прогреется
  i++; 
  tok=read_adc(0);
  
  //цикл, повышающий ток и понижающий частоту ближе к резонансу
  while (tok>30 && tok<315 && OCR2B<110){  //ток больше минимального, т.е. питание силовой подано, и меньше требуемого
   OCR2B++;
   tok_old=tok;
   tok=read_adc(0);
   if (tok_old > tok)errors++; //считаем количество ошибок, когда изменение частоты приводит к изменению тока не в ту сторону (прошли через резонанс)
   if (errors > MAX_ERRORS){   //и если ошибок слишком много - уходим в защиту
    OCR2B=0;
  ...
  //если ток превышает, повышаем частоту дальше от резонанса
   while (tok>325 && OCR2B>=0) {
   OCR2B--;
   tok_old=tok;
   tok=read_adc(0);
   if (tok_old < tok) errors++; //считаем количество ошибок
   if (errors > MAX_ERRORS){    //и если слишком много - уходим в защиту
   OCR2B=0;
   printf("Error: Auto-Ajust Failed");
   delay_ms(500);
   RELAY_PWR_OFF;
  ...
  if (tok<30){
  OCR2B=0; //если пропало силовое питание - выводим частоту на начальный уровень (максимум)
  ...
  if (fet_temp > 60)
  {
    OCR2B=0; //завышаем частоту = занижаем ток
    delay_ms(500); //ждем реакции
    RELAY_PWR_OFF; //отрубаем питание силовой
 ...
 if ((OCR2B < 30) || (OCR2B > 200)){    //если прогрев прошел, но устойчивый OCR2B не попадает в диапазон, тоже отключаем - нужна ручная подстройка базовой частоты
   OCR2B=0;
 ...
 //===ЭТАП ВТОРОЙ. Перестройка частоты под образец
 //повышаем частоту (понижаем ток) с запасом, перед внесением образца
  ...
  while(i < MELT_TIME)
  {
    ...
    printf("temp: %iC(FETs), %iC(Water)", fet_temp, water_temp);
    delay_ms(700); //подбор этой паузы, чтобы MELT_TIME в условии цикла соответствовало секундам (чтобы весь цикл выполнялся за 1 секунду)
  }
